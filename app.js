...
function finRenderMovements(){const filter=$('#finMovementFilter').value;let arr=[...db.finance.transactions];if(filter)arr=arr.filter(t=>t.fromId===filter||t.toId===filter);arr.sort((a,b)=>String(b.date).localeCompare(String(a.date)));$('#finMovements').innerHTML=arr.length?arr.slice(0,150).map(t=>{const sign=t.type==='income'?'+':(t.type==='expense'||t.type==='charge')?'-':'';const cls=t.type==='income'?'money-in':(t.type==='expense'||t.type==='charge')?'money-out':'';return `<div class="row"><div><strong>${esc(t.note||t.category||'Movimiento')}</strong><div class="meta">${esc(t.date)} · ${finTxLabel(t)}${t.category?' · '+esc(t.category):''}</div></div><div class="right"><strong class="${cls}">${sign}${money(t.amount)}</strong><div class="row-actions"><button class="danger-btn" onclick="deleteFinTx('${t.id}')">Eliminar</button></div></div></div>`}).join(''):'<p class="muted">No hay movimientos financieros.</p>';}
...
function backupPayload(){return JSON.stringify({...db,_backup:{app:'Flor Morado Muebles',version:'1.4.2',createdAt:new Date().toISOString()}},null,2);}
...
$('#saleDate').value=today();$('#cashDate').value=today();renderAll();updateBackupUi();addSaleItem();calcSale();
if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=1.4.2',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{});
